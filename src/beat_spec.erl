-module(beat_spec).
-export([type_to_spec/2, parse_spec_attr/6, parse_type_value/2]).
...
type_to_spec({attr, Line, _, _, _}=Ast, State) ->
    State1 = beat_to_erl:add_error(State, invalid_type_declaration, Line, {ast, Ast}),
    R = {atom, Line, error},
    {R, State1}.
...
parse_type_args([Ast|Args], State, Accum) ->
    Line = element(2, Ast),
    State1 = beat_to_erl:add_error(State, invalid_type_argument, Line, {ast, Ast}),
...
parse_type_value(Ast, State) ->
    Line = element(2, Ast),
    State1 = beat_to_erl:add_error(State, invalid_type_value, Line, {ast, Ast}),
    R = {atom, Line, error},
    {R, State1}.
